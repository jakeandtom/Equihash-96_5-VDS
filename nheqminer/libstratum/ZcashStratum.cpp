        ...
        while (true) {
            ...
            while (true) {
				...
				(const std::vector<uint32_t>& index_vector, size_t cbitlen, const unsigned char* compressed_sol) 
				{
					...
					if (compressed_sol)
					{
						actualHeader.nSolution = std::vector<unsigned char>(68);
					...
					uint256 headerhash = actualHeader.GetPoWHash();
...
ZcashJob* ZcashMiner::parseJob(const Array& params)
{
    ...
    if (ret->header.nVersion > 0) {
        ...
        ret->time = params[6].get_str();
        ret->clean = params[10].get_bool();
    } else {
